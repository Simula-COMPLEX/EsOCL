...
public class BranchDistanceCaculation {
	...
	public double handleBooleanOp(IModelInstanceObject env,
		...
		EList<EObject> contents = opCallexp.eContents();
		OperationCallExpImpl A_exp = (OperationCallExpImpl) contents.get(0);
		OperationCallExpImpl B_exp = (OperationCallExpImpl) contents.get(1);
	...
	public double handleCompareOp(IModelInstanceObject env,
		...
		if (oclExpUtility.isComplexType(leftExp).equals(
				OCLExpUtility.OP_COMPLEX_SELECT_SIZE)) {
	...
	private double classifyValue(IModelInstanceObject env,
			OperationCallExpImpl A_exp, OperationCallExpImpl B_exp,
			String opName) {
