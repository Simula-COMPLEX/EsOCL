...
public class BDC4CheckOp {
    ...
    public double handleCheckOp(IModelInstanceObject env,
        ...
        BDC4CompareOp bdc4CompOp = new BDC4CompareOp(interpreter);
        ...
        Collection<IModelInstanceElement> leftInsCol = oclExpUtility.getResultCollection(
                interpreter.doSwitch(contents.get(0)));
        ...
        switch (opName) {
            ...
            case "excludes": {
                ...
                for (int i = 0; i < leftColSize; i++) {
                    ...
                    double leftResult = oclExpUtility.getResultNumericValue((IModelInstanceElement) leftInsCol
                            .toArray()[i]);

                    double rightResult = oclExpUtility.getResultNumericValue(interpreter.doSwitch(contents.get(1))
            ...
            case "includesAll":
            case "excludesAll": {
                double inAll_temp = 0.0;
                double exAll_temp = 0.0;

                Collection<IModelInstanceElement> rightInsCol = oclExpUtility
                        .getResultCollection(this.interpreter.doSwitch(contents.get(1)));
                ...
                for (int i = 0; i < leftColSize; i++) {
                    ...
                    for (int j = 0; j < rightColSize; j++) {
                        ...
                        double rightResult_j = oclExpUtility
                                .getResultNumericValue((IModelInstanceElement) rightInsCol.toArray()[j]);

                        // for includesAll expression
                        double temp_j = bdc4CompOp.compareOp4Numeric(leftResult_i, rightResult_j, "=");
