...
public class OCLSolver {
    ...
    public static void main(String[] args) {
        ...
        try {
            boolean result = oclSolver.solveConstraint(inputModel, constraint);
    ...
    public boolean solveConstraint(File inputModel, String constraint) throws Exception {
    ...
    public boolean getValidInstance(IModel model, List<Constraint> constraintList, String inputModelPath) {

        Search search = new AVM();
        search.setMaxIterations(5000);
        ...
        if (problem.getFitness(v) != 0d) {
            ...
            problem = new OclUmlProblem((UML2Model) model, constraintList);

            problem.processProblem(new File(inputModelPath));

            Search.validateConstraints(problem);

            v = search.search(problem);
